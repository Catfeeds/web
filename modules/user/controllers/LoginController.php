...
//            var_dump($userPass);die;
            $roleArr = "1,2";
//            findBySql("SELECT * FROM {{%user} where userName=$userName and password=$userPass}")
            $loginsdata =  $logins->findBySql("SELECT * FROM {{%user}} where username='".$userName."' and password='".$userPass."' and roleId in ($roleArr) ")->one();
