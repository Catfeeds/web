...
use app\modules\goods\models\Flower;
use app\modules\goods\models\FlowerCategory;
use yii;
use app\libs\AppControl;
class FlowerController extends AppControl {
    function init(){
        parent::init();
    }
    ...
    public function actionAdd(){
        if($_POST){
            ...
            $model->save();
            foreach($catId as $v){
                $model1 = new FlowerCategory();
                $model1->flowerId = $model->primaryKey;
                $model1->catId = $v;
                $model1->save();
            }
            Flower::updateAll(['sort' => $model->primaryKey,'goodsNumber' => (1000+$model->primaryKey)],"id=$model->primaryKey");
            $this->redirect('/goods/flower/index');
        }else{
            $model = new Category();
            $category = $model->getAllCate(0,1);
    ...
    public function actionUpdate(){
        if($_POST){
            ...
            $data = Yii::$app->request->post('data');
            $catId = $data['catId'];
            FlowerCategory::deleteAll("flowerId=$id");
            unset($data['catId']);
            foreach($catId as $v){
                $model1 = new FlowerCategory();
                $model1->flowerId = $id;
            ...
            Flower::updateAll($data,"id=$id");
            $this->redirect('/goods/flower/index');
        }else{
            $id = Yii::$app->request->get('id');
            $data = Flower::find()->asArray()->where("id=$id")->one();
            $data['imageArr'] = explode("-",$data['imageStr']);
            $catStr = "SELECT GROUP_CONCAT(fc.catId) catStr  FROM {{%flower_category}} fc WHERE flowerId = $id GROUP BY fc.flowerId";
    ...
    public function actionFlowerStatus(){
        $id = Yii::$app->request->get('id');
        $sign = Flower::findOne($id);
        if($sign->status == 1){
            Flower::updateAll(['status' => 2],"id=$id");
        }else{
            Flower::updateAll(['status' => 1],"id=$id");
        }
        $this->redirect('/goods/flower/index');
    }
